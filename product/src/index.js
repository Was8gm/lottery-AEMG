  ...
  document.querySelector("#menu").addEventListener("click", function (e) {
    ...
    if (isLotting) {
      if (e.target.id === "lottery") {
        rotateObj.stop();
        btns.lottery.innerHTML = "Lottery";
      } else {
        addQipao("Still in the lottery");
      }
      return false;
    }
    ...
    switch (target) {
      ...
      case "enter":
        removeHighlight();
        addQipao(`The next prize is [${currentPrize.title}], don't leave`);
        // rotate = !rotate;
        rotate = true;
        switchScreen("lottery");
        break;
      ...
      case "reset":
        let doREset = window.confirm(
          "Do you confirm the reset data? After the reset, all the prizes currently drawn will be cleared?"
        );
        if (!doREset) {
          return;
        }
        addQipao("Reset all the data, relottery");
        addHighlight();
        resetCard();
        // 重置所有数据
        currentLuckys = [];
      ...
      case "lottery":
        ...
        addQipao(`Now lottery [${currentPrize.title}], are you ready?`);
        break;
      // 重新抽奖
      case "reLottery":
        if (currentLuckys.length === 0) {
          addQipao(`There is currently no lottery result and cannot be lottery again~~`);
          return;
        }
        setErrorData(currentLuckys);
        addQipao(`Relottery [${currentPrize.title}],prepare!`);
        setLotteryStatus(true);
      ...
      case "save":
        saveData().then(res => {
          ...
          exportData();
          addQipao(`The data has been saved in the EXCEL file`);
        });
        break;
    }
  });
...
function createCard(user, isBold, id, showTable) {
  ...
  if (isBold) {
    ...
    "rgba(255, 195, 91," + (Math.random() * 0.7 + 0.55) + ")";
  }
  //添加公司标识
...
function lottery() {
  ...
  rotateBall().then(() => {
    ...
    if (leftCount < perCount) {
      addQipao("There are not enough people left to participate in the lottery. Now all people can be reset to participate in the second lottery!");
...
function shine(cardIndex, color) {
  ...
  color || "rgba(253, 175, 37," + (Math.random() * 0.9 + 0.15) + ")";
}
    ...
    function (e) {
      if (music.paused) {
        music.play().then(
          ...
          () => {
            addQipao("Failed to automatically play background music, please play it manually!");
          }
        );
      } else {
        music.pause();
